...
- [module3: 病人役割](#module3-病人役割)
  ...
  - [ルールの定義](#ルールの定義-2)
    - [エージェント移動ルール](#エージェント移動ルール-2)
    - [回復ルール](#回復ルール)
  - [役割の定義](#役割の定義-2)
    - [病人役割](#病人役割)
- [シミュレーション定数の定義](#シミュレーション定数の定義-3)
- [ルールの定義](#ルールの定義-3)
  - [健康状態決定ルール](#健康状態決定ルール)
- [役割の定義](#役割の定義-3)
  - [共通役割](#共通役割)
...
sample11では以下のルールを定義する．

- TRuleOfDeterminingHealth：健康状態決定ルール

### 健康状態決定ルール

sample04の健康状態決定ルールと同様．
...
基本的には，それぞれのモジュールの役割を使用するだけだが，それぞれのモジュールで定義されているAgentMovingステージを，
TSOARSBuilder.mergeStagesメソッドでマージすることで同じステージとして解釈されるように設定している．
これにより，全てのAgentMovingがmodule1のAgentMovingに統一され，例えば，module2のAgentMovingを指定してもmodule1のAgentMovingとして処理される．
...
public class TMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<Enum<?>> stages = List.of(EStage.DeterminingHealth,
                jp.soars.samples.sample11.module1.EStage.AgentMoving,
                jp.soars.samples.sample11.module2.EStage.AgentMoving,
                jp.soars.samples.sample11.module3.EStage.AgentMoving); // ステージリスト
        ...
        // ログ出力設定
        String pathOfLogDir = "logs" + File.separator + "sample11"; // ログディレクトリ
        ...
        // ステージをマージして同一ステージとする．
        // これにより，どのEStageで定義されたAgentMovingを指定しても同じものとして解釈される．
        builder.mergeStages(jp.soars.samples.sample11.module1.EStage.AgentMoving,
                            jp.soars.samples.sample11.module2.EStage.AgentMoving,
                            jp.soars.samples.sample11.module3.EStage.AgentMoving);
        ...
        for (int i = 0; i < fathers.size(); ++i) {
            ...
            TRole commonRole = new TRoleOfCommon(father, home, jp.soars.samples.sample11.module1.ERoleName.Father); // 共通役割を生成する．
            TRole fatherRole = new TRoleOfFather(father, home, company); // 父親役割を生成する．
            new TRoleOfSickPerson(father, home, hospital, new TTime("2:00:00"), jp.soars.samples.sample11.module1.ERoleName.Father); // 病人役割を生成する．診察時間は2時間とする．
